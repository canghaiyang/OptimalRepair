...
int main(int argc, char *argv[])
{
    if (argc < 2)
    {
        ...
        printf("Error format\n");
        printf("./cut_file buffer_size(KB) final_size(KB) src_filename dst_filename\n");
        return 0;
    }
    int buffer_size_KB = atoi(argv[1]);
    int final_size_KB = atoi(argv[2]);
    if (final_size_KB % buffer_size_KB != 0)
    {
    ...
    int times = final_size_KB / buffer_size_KB;
    int buffer_size = buffer_size_KB * 1024;               // Byte
    ...
    for (i = 0; i < times; i++)
    {
        ...
        {
            printf("Error src_file: src_file size less %d KB\n", final_size_KB);
            fclose(src_fp);
            fclose(dst_fp);
            return -1;
        }
    ...
    fclose(src_fp);
    fclose(dst_fp);
    printf("Successfully: dst_file size %d KB\n", final_size_KB);
    return 0;
}